...
Como se puede ver en los argumentos, es necesario darle la tabla de `datos` y el nombre del indicador `indName` (El de la tabla `tbl.specs`) y opcionalmente se pueden dar lista para los parámetros `parm` y `sigParm`. La ventaja de esta función es que nos permite realizar tunning de los parametros. 
